...
# 表示順（サイドバーのチェックボックス順にも使う）。「その他」（助詞・記号・空白等、
# map_pos()のフォールバック——下記参照）はここに含めない——選択可能にすると、記号・空白
# トークンがワードクラウドの語として渡り、PillowのImageDraw.textbboxが改行文字混じりの
# 語で例外（ValueError）を投げる不具合が実機で発生したため。
CATEGORY_ORDER = [
    '強制抽出',
    '名詞', '固有名詞', 'サ変名詞', '名詞B',
    '動詞', '動詞B',
