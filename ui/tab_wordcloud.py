...
COLOR_MODES = ['品詞別4色', 'モノクロ3段階ネガポジ（実験）']
...
def render(tokens: list, included_categories: set, stopwords: set[str] | None = None) -> None:
    ...
    bold = st.checkbox('文字を太くする', value=True)
    ...
    wc = generate_wordcloud(freq, mask_shape=mask_shape, color_func=color_func, bold=bold)
    ...
    if st.button('PNGを生成してダウンロード'):
        ...
        wc_hires = generate_wordcloud(freq, mask_shape=mask_shape, color_func=color_func, bold=bold, scale=scale)
